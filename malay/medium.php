...
<div class="w3-top">
	...
	<center><h1>Assessment 2 <span class="qz"> / Penilaian 2</span></h1></center>
	<br>
	<center><h3>Level: Medium</h3></center>
	
	
	
	<div class="card mb-3">
		<?php 
    $quiz1 = "SELECT * from questiontable q, audio a WHERE a.audioid = q.audioid AND assid = '".$assid."' limit 1";
    $quizres = mysqli_query($conn,$quiz1);
		
		if ($quizres && mysqli_num_rows($quizres)>0)
		{
			while($row = mysqli_fetch_assoc($quizres)) 
			{ ?>

					<form action="evaluatemedium.php" method="POST" >
						<div class="card-body">
