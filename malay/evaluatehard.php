...
<div class="w3-top">
	...
	<center><h1>Assessment 3 <span class="qz"> / Penilaian 3</span></h1></center>
	<br>
	<center><h3>Level: Hard</h3></center>
	
	<div class="card mb-3">
	<?php
 	$quiz3 = "SELECT * FROM questiontable q JOIN image i ON q.imageid = i.imageid JOIN audio a ON a.audioid = q.audioid WHERE q.assid = '".$assid."'AND questionID > $questionID LIMIT 1";
	 $quiz3res = mysqli_query($conn,$quiz3);
		 ...
		 if ($quiz3res && mysqli_num_rows($quiz3res)>0)
		 {
			 ...
			 { ?>
					 ...
					 <form action="evaluatehard.php" method="POST" >
						 <div class="card-body">
			...
			if($result1)
			{
				if(mysqli_num_rows($result1)>0)
				{
					...
					if($resu)
					{
						if($mark >= 80)
						{
							echo "<script>alert('Congrats. You get $mark%. You have passed the assessment!')</script>";
							echo"<meta http-equiv='refresh' content='0; url=assessment.php?id=5003'/>";
						}
						else if ($mark <80)
						{
